...
2. **Find the cheapest ground-truth** for it. Free public data, a held-out slice, a
   ...
   (a free public dataset pull; 12 `claude -p` cells). Ask: what's the least I can gather that could
   prove this false?
...
4. **Run it and compute uncertainty.** Report rates with confidence intervals, not point
   ...
   Compute the CI on the *independent unit* (window, user, day), not the raw row count —
   correlated rows inflate n and manufacture false precision.
...
- **Pseudo-replication / unnamed selection:** CIs computed on correlated rows (12,000 trades
  from 250 windows) — effective n is the independent cluster, not the row count; and reporting
  the best-of-N slice as if it were the only hypothesis. Compute on the independent unit; name
  the search space.
- **Ephemeral evidence:** a verdict resting on data that can't be re-pulled (a live API with
  short retention) and was never snapshotted — commit the raw data, or label it "unreproducible
  after DATE."
