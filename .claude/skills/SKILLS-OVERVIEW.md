...
A portable, project-agnostic quality pack: 35 skills that improve **process** — planning, verification, editing, debugging, decision-making — on any project. 18 form the core rigor set; 10 were added in an expansion pass for high-recurrence domains (codebase work, error recovery, delegation, research, prompts, ideation, product decisions, the public boundary, human handoff, and open mandates); `extract-approach` and `session-orientation` add the persistence and continuity layers; `empirical-validation` adds the test-the-claim-with-data layer; `leverage-first` adds the find-the-efficient-path-before-grinding reflex; and `correction-propagation`, `security-pattern-review`, and `disclosure-is-not-a-fix` add the ship-integrity layer (from a 2026-07-09 cross-repo audit of what the pack itself shipped). They add discipline, not capability; see `HOW-TO-USE-WITH-OPUS.md` for limitations.
