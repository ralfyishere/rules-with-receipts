...
## What this is
A quality pack for Claude Code (29 skills + operating manual + always-on rules snippet) that ships with its own eval evidence. It improves process discipline (scope, verification, recovery), not model capability. The evidence, including unflattering findings, is in `eval-results/` and `eval-results-v2/`.
