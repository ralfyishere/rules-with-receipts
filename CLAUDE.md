...
- **Changing a proven component requires an A/B, not just a count check.** Before adding to / rewording the snippet, an exemplar, or anything with measured effect, run a controlled experiment (isolate the one change, `eval-results-v2/ab-snippet-t04.sh` is the template) — attention budget is scarce and a "good" addition can dilute the proven effect (learned the hard way: the session-orientation snippet rule cut t04 flagging ~9/12 → ~1/9). A new skill can ship as a skill without also claiming an always-on slot.
...
- **Continuity layer:** `ACTIVE-PROJECTS.md` (state registry) + `SESSION-START.md` (orientation procedure) are private, never mirrored. Read the registry at session start; update the touched entries at closeout of any session that changes project state. Durable work never stays in session scratchpads.
